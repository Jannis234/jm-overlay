...
RDEPEND=">=dev-qt/qtcore-5.14.2:5=
	>=dev-qt/qtgui-5.14.2:5=
	>=dev-qt/qtconcurrent-5.14.2:5=
	>=dev-qt/qtdbus-5.14.2:5=
...
pkg_pretend() {
	if [[ "${MERGE_TYPE}" != "binary" ]]; then
		if tc-is-clang; then
			[[ $(clang-major-version) -lt 9 ]] && die "RPCS3 needs >=sys-devel/clang-9 to build"
		elif tc-is-gcc; then
			[[ $(gcc-major-version) -lt 9 ]] && die "RPCS3 needs >=sys-devel/gcc-9 to build"
		else
			die "RPCS3 needs  >=sys-devel/clang-9 or >=sys-devel/gcc-9 to build"
		fi
	fi
}
